...
public class SeaPort {
    ...
    public void requestDock(Ship ship) throws ResourceException {
        ...
        LOGGER.info("Ship № {} got a dock № {}, {} docks available, {} ships in queue",
                ship.getShipId(), dock.getDockId(), freeDocks.size(), dockSemaphore.getQueueLength());
    }

    public void leaveDock(int dockId) {
        Optional<Dock> optionalDock =
                usedDocks.stream().filter(d -> d.getDockId() == dockId).findFirst();
